...
def _build_clubs_payload(db: Dict[str, Any], division_rows: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    for row in rows:
        ...
        row["breakdown"].sort(key=lambda x: (-float(x.get("awarded_points") or 0.0), x.get("full_name", "").lower()))

    rows.sort(key=lambda x: (-float(x["points"]), x["participants_count"], -int(x["first_places"]), x["club_name"].lower()))
